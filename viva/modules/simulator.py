...
class Simulator:
    ...
    def update_view(
        self, x: float, y: float, z: float, roll: float, pitch: float, yaw: float, frame: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, StateElement, np.ndarray]:
        """Updates the drone view based on external state (bypassing internal physics)."""
        
        self.drone.state[0].pos.x = x
        self.drone.state[0].pos.y = y
        self.drone.state[0].pos.z = z
        self.drone.roll_deg = roll
        self.drone.pitch_deg = pitch
        self.drone.psi_deg = yaw
        ...
        nadir_point = self.camera.world_to_pixel(x, y)
        return drone_view, points, self.drone.state[0], nadir_point.astype(np.int32)
